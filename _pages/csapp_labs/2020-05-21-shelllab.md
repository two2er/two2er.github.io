...
在接收到`SIGINT`信号以后，主进程像前台进程组发送`SIGINT`信号；收到信号的子进程终止，像主进程发送`SIGCHLD`信号。它们在作业列表中的操作交由`SIGCHLD`的信号处理程序处理。`SIGTSTP`信号同理，在主进程接收到信号以后暂停前台进程组。暂停以后，前台进程组在工作列表中的变化也交由`SIGCHLD`信号处理程序处理。
